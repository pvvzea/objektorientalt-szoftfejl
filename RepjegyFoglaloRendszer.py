...
from abc import ABC, abstractmethod #osztályok létrehozásához
from datetime import datetime #idő kezelése
...
class LegiTarsasag:
    def __init__(self, nev):
        self.nev = nev
        self.jaratok = [] # járatok tömbje
        self.foglalasok = [] # foglalások tömbje
...
def rendszer_inditas():
    legi_tarsasag = LegiTarsasag("Malev")

    #Előre létrehozott járatok
    belfoldi_jarat1 = BelfoldiJarat("B001", "Budapest", 5000)
    belfoldi_jarat2 = BelfoldiJarat("B002", "Debrecen", 6000)
    belfoldi_jarat3 = BelfoldiJarat("B003", "Sármellék", 4000)
    nemzetkozi_jarat1 = NemzetkoziJarat("N001", "London", 15000)
    nemzetkozi_jarat2 = NemzetkoziJarat("N002", "New York", 32000)
    nemzetkozi_jarat3 = NemzetkoziJarat("N003", "Berlin", 14000)

    legi_tarsasag.jarat_hozzaadasa(belfoldi_jarat1)
    legi_tarsasag.jarat_hozzaadasa(belfoldi_jarat2)
    legi_tarsasag.jarat_hozzaadasa(belfoldi_jarat3)
    legi_tarsasag.jarat_hozzaadasa(nemzetkozi_jarat1)
    legi_tarsasag.jarat_hozzaadasa(nemzetkozi_jarat2)
    legi_tarsasag.jarat_hozzaadasa(nemzetkozi_jarat3)

    #Előre létrehozott foglalások
    foglalas1 = JegyFoglalas("F001", belfoldi_jarat1, "Kiss Barbara", datetime.now())
    foglalas2 = JegyFoglalas("F002", belfoldi_jarat2, "Nagy János", datetime.now())
    foglalas3 = JegyFoglalas("F003", belfoldi_jarat3, "Horváth Lili", datetime.now())
    foglalas4 = JegyFoglalas("F004", nemzetkozi_jarat1, "Szabó Dóra", datetime.now())
    foglalas5 = JegyFoglalas("F005", nemzetkozi_jarat2, "Kelemen András", datetime.now())
    foglalas6 = JegyFoglalas("F006", nemzetkozi_jarat3, "Tóth István", datetime.now())
    ...
    legi_tarsasag.foglalas_hozzaadasa(foglalas4)
    legi_tarsasag.foglalas_hozzaadasa(foglalas5)
    legi_tarsasag.foglalas_hozzaadasa(foglalas6)
...
#Egyszerű konzolos felhasználói interfész a jegyfoglalások kezeléséhez.
#Fő program
def main():
    ...
    #Üdvözlés
    print("***********************************")
    print(f"Üdvözöljük a {legi_tarsasag.nev} légitársaságnál!")
    print("Válasszon a menüpontokból!")
    print("***********************************")
